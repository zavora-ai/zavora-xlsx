...
fn main() -> Result<()> {
    ...
    // Sheet 1: Review Data
    {
        ...
        r += 1;
        ws.write_with_format(r, 1, "⭐ Performance Reviews", &title)?;
        ws.set_row_height(r, 32.0)?;
        r += 1;
        ws.write_with_format(
            r,
            1,
            "Annual Review Cycle — Q1 2025",
    ...
    for (c, w) in cw2.iter().enumerate() {
        ws2.set_column_width(c as u16, *w)?;
    }

    let title2 = Format::new()
        .bold()
        .font_size(18.0)
        .font_color(navy)
        .align(Align::Left)
    ...
    let mut r2 = 1u32;
    ws2.write_with_format(r2, 1, "Performance Summary", &title2)?;
    r2 += 3;

    ws2.merge_range(r2, 1, r2, 6, "  Department Averages", &sec)?;
    ws2.set_row_height(r2, 26.0)?;
    r2 += 1;
    let dh = [
        "Department",
        "Technical",
    ...
    for (c, h) in dh[1..].iter().enumerate() {
        ws2.write_with_format(r2, (c + 2) as u16, *h, &hdr2)?;
    }
    r2 += 1;
    ...
    for dept in &depts {
        ...
        for c in 0..4u16 {
            ...
            ws2.write_formula(
                ...
                c + 2,
                &format!(
                    "AVERAGEIF(Reviews!C7:C18,B{},Reviews!{}7:{}18)",
                    r2 + 1,
                    col_letter,
                    col_letter
                ),
    ...
    chart.set_title("Department Performance");
    chart.set_width(520);
