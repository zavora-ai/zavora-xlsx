...
fn main() -> Result<()> {
    ...
    macro_rules! wl {
        ($r:ident, $name:expr, $cy:expr, $py:expr, $alt:expr) => {{
            ...
            ws.write_formula($r, 4, &format!("C{}-D{}", $r + 1, $r + 1))
                .unwrap();
            ws.set_cell_format($r, 4, if $cy >= $py { &chg_pos } else { &chg_neg })
                .unwrap();
            ws.write_formula($r, 5, &format!("IFERROR(E{}/D{},0)", $r + 1, $r + 1))
                .unwrap();
        ...
        ($r:ident, $name:expr, $start:expr, $end:expr) => {{
            ...
            for c in 2..=3u16 {
                ws.write_formula(
                    $r,
                    c,
                    &format!("SUM({}{}:{}{})", col(c), $start + 1, col(c), $end + 1),
                )
                .unwrap();
                ws.set_cell_format($r, c, &stv).unwrap();
            }
            ws.write_formula($r, 4, &format!("C{}-D{}", $r + 1, $r + 1))
                .unwrap();
            ws.set_cell_format($r, 4, &stv).unwrap();
            ws.write_formula($r, 5, &format!("IFERROR(E{}/D{},0)", $r + 1, $r + 1))
                .unwrap();
            ws.set_cell_format($r, 5, &pct_fmt).unwrap();
        }};
        ...
        ($r:ident, $name:expr, $color:expr, $r1:expr, $r2:expr) => {{
            ...
            for c in 2..=3u16 {
                ws.write_formula(
                    $r,
                    c,
                    &format!("{}{}+{}{}", col(c), $r1 + 1, col(c), $r2 + 1),
                )
                .unwrap();
                ws.set_cell_format($r, c, &gv).unwrap();
            }
            ws.write_formula($r, 4, &format!("C{}-D{}", $r + 1, $r + 1))
                .unwrap();
            ws.set_cell_format($r, 4, &gv).unwrap();
            ws.write_formula($r, 5, &format!("IFERROR(E{}/D{},0)", $r + 1, $r + 1))
                .unwrap();
            ws.set_cell_format($r, 5, &pct_fmt).unwrap();
    ...
    r += 1;
    ws.write_with_format(r, 1, "Balance Sheet", &title)?;
    ws.set_row_height(r, 32.0)?;
    r += 1;
    ws.write_with_format(
        r,
        1,
        "As of December 31, 2025  |  Comparative with Prior Year",
        &sub,
    )?;
    r += 1;
    for c in 1..=5u16 {
    ...
    ws.write_with_format(r, 2, "FY 2025", &hdr)?;
    ws.write_with_format(r, 3, "FY 2024", &hdr)?;
    ws.write_with_format(r, 4, "Change", &hdr)?;
    ws.write_with_format(r, 5, "% Change", &hdr)?;
    ...
    let ca_e = r - 1;
    subtotal!(r, "Total Current Assets", ca_s, ca_e);
    let tca = r;
    r += 1;
    ...
    let nca_e = r - 1;
    subtotal!(r, "Total Non-Current Assets", nca_s, nca_e);
    let tnca = r;
    r += 1;

    grand!(r, "TOTAL ASSETS", blue, tca, tnca);
    let ta = r;
    r += 2;
    ...
    let cl_e = r - 1;
    subtotal!(r, "Total Current Liabilities", cl_s, cl_e);
    let tcl = r;
    r += 1;
    ...
    let ncl_e = r - 1;
    subtotal!(r, "Total Non-Current Liabilities", ncl_s, ncl_e);
    let tncl = r;
    r += 1;

    grand!(r, "TOTAL LIABILITIES", red, tcl, tncl);
    let tl = r;
    r += 2;
    ...
    for c in 2..=3u16 {
        ws.write_formula(
            r,
            c,
            &format!("SUM({}{}:{}{})", col(c), eq_s + 1, col(c), eq_e + 1),
        )?;
        ws.set_cell_format(r, c, &gv)?;
    }
    ws.write_formula(r, 4, &format!("C{}-D{}", r + 1, r + 1))?;
    ws.set_cell_format(r, 4, &gv)?;
    ws.write_formula(r, 5, &format!("IFERROR(E{}/D{},0)", r + 1, r + 1))?;
    ws.set_cell_format(r, 5, &pct_fmt)?;
    ...
    for c in 2..=3u16 {
        ws.write_formula(r, c, &format!("{}{}+{}{}", col(c), tl + 1, col(c), te + 1))?;
        ws.set_cell_format(r, c, &cv)?;
    }
    ws.write_formula(r, 4, &format!("C{}-D{}", r + 1, r + 1))?;
    ws.set_cell_format(r, 4, &cv)?;
    ws.write_formula(r, 5, &format!("IFERROR(E{}/D{},0)", r + 1, r + 1))?;
    ws.set_cell_format(r, 5, &pct_fmt)?;
    ws.set_row_height(r, 28.0)?;
    r += 1;
    ...
    ws.write_with_format(
        ...
        1,
        "Balance Check",
        &Format::new().italic().font_size(10.0).font_color("#667085"),
    )?;
    ws.write_formula(
        r,
        2,
        &format!(
            "IF(C{}=C{},\"✅ Balanced\",\"❌ Out of Balance\")",
            ta + 1,
            tle + 1
        ),
    )?;
    ...
    // Chart
    let src = r + 15;
    ws.write(src, 1, "Assets")?;
    ws.write_formula(src, 2, &format!("C{}", ta + 1))?;
    ws.write(src + 1, 1, "Liabilities")?;
    ws.write_formula(src + 1, 2, &format!("C{}", tl + 1))?;
    ws.write(src + 2, 1, "Equity")?;
    ws.write_formula(src + 2, 2, &format!("C{}", te + 1))?;
    ...
    s.set_point_color(0, blue);
    s.set_point_color(1, red);
    s.set_point_color(2, green);
    ...
    let path = std::path::PathBuf::from("output/balance_sheet.xlsx");
    wb.save(&path)?;
