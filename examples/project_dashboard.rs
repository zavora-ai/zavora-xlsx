...
fn main() -> Result<()> {
    ...
    // ═══════════════════════════════════════════════════════════
    {
        ...
        for (i, a) in activities.iter().enumerate() {
            ...
            ws.write_with_format(
                r,
                3,
                ExcelDateTime::from_ymd(a.start.0, a.start.1, a.start.2).unwrap(),
                &df,
            )?;
            ws.write_with_format(
                r,
                4,
                ExcelDateTime::from_ymd(a.end.0, a.end.1, a.end.2).unwrap(),
                &df,
            )?;
    ...
    let cw = [
        2.0, 14.0, 16.0, 14.0, 12.0, 12.0, 10.0, 4.0, 14.0, 14.0, 14.0, 14.0, 2.0,
    ];
    ...
    let tbl_hdr_l = Format::new()
        ...
        .align(Align::Left)
        .border(BorderStyle::Thin);
    let cell_l = Format::new()
        .font_size(10.0)
        .font_color(navy)
        .align(Align::Left)
        .border(BorderStyle::Thin)
        .border_color(border);
    let cell_c = Format::new()
        .font_size(10.0)
        .font_color(navy)
        .align(Align::Center)
        .border(BorderStyle::Thin)
        .border_color(border);
    let cell_pct = Format::new()
        .font_size(10.0)
        .font_color(navy)
        .num_format("0%")
        .align(Align::Center)
    ...
    for (c, h) in ph[1..].iter().enumerate() {
        ws2.write_with_format(r, (c + 2) as u16, *h, &tbl_hdr)?;
    }
    ...
    for person in &people {
        ...
        ws2.write_formula(
            r,
            5,
            &format!("IF(E{r1}>=1,\"✅\",IF(E{r1}>0,\"🔄\",\"⏳\"))", r1 = r + 1),
        )?;
    ...
    ws2.add_conditional_format(
        ...
        r - 1,
        2,
        ConditionalFormatDataBar::new(red),
    )?;
    // Data bars on avg %
    ws2.add_conditional_format(
        people_start,
    ...
    for (ci, &(cat, color)) in cat_colors.iter().enumerate() {
        ...
        ws2.write_formula(
            cr2,
            10,
            &format!("IFERROR(AVERAGEIF({cr},I{},{dr}),0)", cr2 + 1),
        )?;
    ...
    ps.set_values(&format!(
        "'Project Dashboard'!$J${}:$J${}",
        cat_start + 1,
        cat_end + 1
    ));
    ps.set_categories(&format!(
        "'Project Dashboard'!$I${}:$I${}",
        cat_start + 1,
        cat_end + 1
    ));
    ps.set_data_labels(true);
    for (i, &(_, color)) in cat_colors.iter().enumerate() {
        ps.set_point_color(i, color);
    }
    ...
    bs.set_values(&format!(
        "'Project Dashboard'!$D${}:$D${}",
        people_start + 1,
        people_start + people.len() as u32
    ));
    bs.set_categories(&format!(
        "'Project Dashboard'!$B${}:$B${}",
        people_start + 1,
        people_start + people.len() as u32
    ));
    ...
    let path = std::path::PathBuf::from("output/project_dashboard.xlsx");
    wb.save(&path)?;
