...
fn main() -> Result<()> {
    ...
    // ═══════════════════════════════════════════════════════════
    {
        ...
        let incomes = [
            ("Salary", 8500.0, 0.05),
            ("Side Business", 2000.0, 0.15),
            ("Investments", 800.0, 0.10),
            ("Rental Income", 1500.0, 0.03),
        ...
        for (i, (name, monthly, growth)) in incomes.iter().enumerate() {
            ...
            ws.write_formula(r, 2, &format!("B{}*12", r + 1))?;
            ws.set_cell_format(r, 2, &mf)?;
        ...
        for (c, h) in h2.iter().enumerate() {
            ws.write_with_format(goals_start, c as u16, *h, &hf)?;
        }
        ...
        for (i, (name, target, current, deadline)) in goals.iter().enumerate() {
            ...
            ws.write_formula(r, 3, &format!("C{}/B{}", r + 1, r + 1))?;
            ws.set_cell_format(r, 3, &pf)?;
            ws.write_with_format(
                r,
                4,
                ExcelDateTime::from_ymd(deadline.0, deadline.1, deadline.2).unwrap(),
                &df,
            )?;
            ...
            ws.write_formula(
                ...
                5,
                &format!(
                    "IFERROR((B{r1}-C{r1})/MAX(1,(E{r1}-TODAY())/30.44),0)",
                    r1 = r + 1
                ),
            )?;
        ...
        for (c, h) in h3.iter().enumerate() {
            ws.write_with_format(nw_start, c as u16, *h, &hf)?;
        }

        let assets = [
            ("Checking Account", 12000.0, "Asset"),
            ("Savings Account", 35000.0, "Asset"),
            ("Investment Portfolio", 280000.0, "Asset"),
            ("Rental Property", 320000.0, "Asset"),
            ("Car", 25000.0, "Asset"),
            ("401k/Retirement", 180000.0, "Asset"),
            ("Mortgage", -245000.0, "Liability"),
            ("Car Loan", -18000.0, "Liability"),
            ("Student Loans", -32000.0, "Liability"),
            ("Credit Cards", -4500.0, "Liability"),
        ];
    ...
    ws2.write_with_format(
        r,
        1,
        "Your roadmap to financial freedom  |  Auto-updates from Data sheet",
        &subtitle,
    )?;
    r += 1;
    for c in 1..=8u16 {
    ...
    ws2.write_formula(
        r,
        4,
        &format!("SUM(Data!B{}:B{})", nw_start + 2, nw_last + 1),
    )?;
    ...
    ws2.write_formula(
        ...
        5,
        &format!(
            "SUMIF(Data!C{}:C{},\"Liability\",Data!B{}:B{})",
            nw_start + 2,
            nw_last + 1,
            nw_start + 2,
            nw_last + 1
    ...
    ws2.merge_range(
        ...
        r,
        8,
        "  🎯 Financial Goals — Progress Tracker",
        &section,
    )?;
    ...
    for i in 0..6u32 {
        ...
        ws2.write_formula(r, 1, &format!("Data!A{dr}"))?;
        ws2.set_cell_format(r, 1, &goal_name)?;
        ws2.write_formula(r, 2, &format!("Data!B{dr}"))?;
        ws2.set_cell_format(r, 2, &goal_val)?;
        ws2.write_formula(r, 3, &format!("Data!C{dr}"))?;
        ws2.set_cell_format(r, 3, &goal_val)?;
        ws2.write_formula(r, 4, &format!("Data!D{dr}"))?;
        ws2.set_cell_format(r, 4, &goal_pct)?;
        ws2.write_formula(r, 5, &format!("Data!E{dr}"))?;
        ws2.set_cell_format(r, 5, &goal_date)?;
        ws2.write_formula(r, 6, &format!("Data!F{dr}"))?;
        ws2.set_cell_format(r, 6, &goal_need)?;
    ...
    for i in 0..10u32 {
        let dr = nw_start + 2 + i;
        ws2.write_formula(r + i, 1, &format!("Data!A{dr}"))?;
        ws2.set_cell_format(r + i, 1, &item_fmt)?;
        ws2.write_formula(r + i, 2, &format!("Data!B{dr}"))?;
        // Color based on type
        ws2.write_formula(r + i, 2, &format!("Data!B{dr}"))?;
        ws2.set_cell_format(r + i, 2, if i < 6 { &asset_val } else { &liab_val })?;
        ws2.write_formula(r + i, 3, &format!("Data!C{dr}"))?;
        ws2.set_cell_format(r + i, 3, &type_fmt)?;
    }

    // Income sources (right side)
    for i in 0..5u32 {
        let dr = i + 2;
        ws2.write_formula(r + i, 5, &format!("Data!A{dr}"))?;
        ws2.set_cell_format(r + i, 5, &item_fmt)?;
        ws2.write_formula(r + i, 6, &format!("Data!B{dr}"))?;
        ws2.set_cell_format(r + i, 6, &goal_val)?;
        ws2.write_formula(r + i, 7, &format!("Data!C{dr}"))?;
        ws2.set_cell_format(r + i, 7, &goal_val)?;
        ws2.write_formula(r + i, 8, &format!("Data!D{dr}"))?;
        ws2.set_cell_format(r + i, 8, &goal_pct)?;
    }
...
fn home(name: &str) -> std::path::PathBuf {
    std::path::PathBuf::from("output").join(name)
}
