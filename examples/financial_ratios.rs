...
fn main() -> Result<()> {
    ...
    let navy = "#1B2A4A"; let green = "#0D7C3D"; let red = "#C00000"; let blue = "#2B579A"; let border = "#D6DCE4";
    ...
    // ═══════════════════════════════════════════════════════════
    {
        ...
        let lbl = Format::new().font_size(10.0).font_color(navy).align(Align::Left).border(BorderStyle::Thin).border_color(border);
        ...
        let sec = Format::new().bold().font_size(10.0).font_color(blue).italic().align(Align::Left).border(BorderStyle::Thin).border_color(border);
        ...
        for (i, (name, vals, is_sec)) in data.iter().enumerate() {
            let r = (i + 1) as u32;
            if *is_sec { ws.write_with_format(r, 0, *name, &sec)?; for c in 1..=3u16 { ws.write_with_format(r, c, "", &lbl)?; } }
            else { ws.write_with_format(r, 0, *name, &lbl)?; for (c, v) in vals.iter().enumerate() { ws.write_with_format(r, (c+1) as u16, *v, &val)?; } }
        }
        ws.set_column_width(0, 22.0)?; for c in 1..=3u16 { ws.set_column_width(c, 14.0)?; }
    ...
    let cw = [2.0, 24.0, 14.0, 14.0, 14.0, 4.0, 24.0, 14.0, 14.0, 14.0, 2.0];
    ...
    let section = |c: &str| Format::new().bold().font_size(12.0).font_color("#FFFFFF").background_color(c).align(Align::Left).align(Align::VerticalCenter);
    ...
    let ratio_l = Format::new().font_size(10.0).font_color(navy).bold().align(Align::Left).border(BorderStyle::Thin).border_color(border);
    let ratio_pct = Format::new().font_size(10.0).font_color(navy).align(Align::Center).num_format("0.0%").border(BorderStyle::Thin).border_color(border);
    let ratio_x = Format::new().font_size(10.0).font_color(navy).align(Align::Center).num_format("0.00x").border(BorderStyle::Thin).border_color(border);
    let ratio_num = Format::new().font_size(10.0).font_color(navy).align(Align::Center).num_format("0.00").border(BorderStyle::Thin).border_color(border);

    let mut r = 0u32;
    ws2.set_row_height(r, 6.0)?; r += 1;
    ws2.write_with_format(r, 1, "📊 Financial Ratios Dashboard", &title)?; ws2.set_row_height(r, 32.0)?; r += 1;
    ws2.write_with_format(r, 1, "3-Year Trend Analysis  |  All ratios calculated from Data sheet", &Format::new().font_size(10.0).font_color("#667085").italic())?; r += 1;
    for c in 1..=9u16 { ws2.write_with_format(r, c, "", &divider)?; } ws2.set_row_height(r, 3.0)?; r += 2;
    ...
    for (c, y) in years.iter().enumerate() { ws2.write_with_format(r, (c+2) as u16, *y, &hdr)?; }
    ...
    for (name, num, den) in &profit_ratios {
        ...
        for (c, yc) in yr_cols.iter().enumerate() {
            ws2.write_formula(r, (c+2) as u16, &format!("IFERROR(Data!{yc}{num}/Data!{yc}{den},0)"))?;
            ws2.set_cell_format(r, (c+2) as u16, &ratio_pct)?;
        }
        ws2.set_row_height(r, 22.0)?; r += 1;
    }
    ws2.add_conditional_format(profit_start, 2, r-1, 4, ConditionalFormatDataBar::new(green))?;
    ...
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(liq_start, (c+7) as u16, &format!("IFERROR(Data!{yc}9/Data!{yc}11,0)"))?;
        ws2.set_cell_format(liq_start, (c+7) as u16, &ratio_x)?;
    }
    ws2.write_with_format(liq_start + 1, 6, "Working Capital", &ratio_l)?;
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(liq_start + 1, (c+7) as u16, &format!("(Data!{yc}9-Data!{yc}11)/1000"))?;
        ws2.set_cell_format(liq_start + 1, (c+7) as u16, &Format::new().font_size(10.0).font_color(navy).align(Align::Center).num_format("$#,##0\"k\"").border(BorderStyle::Thin).border_color(border))?;
    }
    ...
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(r, (c+2) as u16, &format!("IFERROR(Data!{yc}12/Data!{yc}13,0)"))?;
        ws2.set_cell_format(r, (c+2) as u16, &ratio_num)?;
    }
    r += 1;
    ws2.write_with_format(r, 1, "Debt Ratio", &ratio_l)?;
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(r, (c+2) as u16, &format!("IFERROR(Data!{yc}12/Data!{yc}10,0)"))?;
        ws2.set_cell_format(r, (c+2) as u16, &ratio_pct)?;
    }
    r += 1;
    ws2.write_with_format(r, 1, "Equity Multiplier", &ratio_l)?;
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(r, (c+2) as u16, &format!("IFERROR(Data!{yc}10/Data!{yc}13,0)"))?;
    ...
    // ── Efficiency ──
    ws2.merge_range(r + 1, 6, r + 1, 9, "  Efficiency Ratios", &section("#8E44AD"))?;
    ws2.write_with_format(r + 2, 6, "Ratio", &hdr_l)?;
    for (c, y) in years.iter().enumerate() { ws2.write_with_format(r + 2, (c+7) as u16, *y, &hdr)?; }

    ws2.write_with_format(r + 3, 6, "Asset Turnover", &ratio_l)?;
    for (c, yc) in yr_cols.iter().enumerate() {
        ws2.write_formula(r + 3, (c+7) as u16, &format!("IFERROR(Data!{yc}2/Data!{yc}10,0)"))?;
    ...
    for (c, yc) in yr_cols.iter().enumerate() {
        ...
        ws2.write_formula(r + 4, (c+7) as u16, &format!("IFERROR(Data!{yc}2/{},0)", emps[c]))?;
        ws2.set_cell_format(r + 4, (c+7) as u16, &Format::new().font_size(10.0).font_color(navy).align(Align::Center).num_format("$#,##0").border(BorderStyle::Thin).border_color(border))?;
    }
    ...
    chart.set_title("Profitability Trend"); chart.set_width(480); chart.set_height(280);
    chart.set_y_axis_name("%"); chart.set_legend_position(LegendPosition::Bottom);

    // Gross Margin series
    let gm = chart.add_series();
    gm.set_values(&format!("'Ratios Dashboard'!$C${}:$E${}", profit_start+1, profit_start+1));
    gm.set_categories("'Ratios Dashboard'!$C$6:$E$6");
    gm.set_name("Gross Margin"); gm.set_color(green);

    let nm = chart.add_series();
    nm.set_values(&format!("'Ratios Dashboard'!$C${}:$E${}", profit_start+2, profit_start+2));
    nm.set_categories("'Ratios Dashboard'!$C$6:$E$6");
    nm.set_name("Net Margin"); nm.set_color(blue); nm.set_data_labels(true);

    let roe = chart.add_series();
    roe.set_values(&format!("'Ratios Dashboard'!$C${}:$E${}", profit_start+3, profit_start+3));
    roe.set_categories("'Ratios Dashboard'!$C$6:$E$6");
    roe.set_name("ROE"); roe.set_color("#E67E22");

    ws2.insert_chart(chart_row, 1, &chart)?;

    // Revenue growth bar
    let mut bar = Chart::new(ChartType::Column);
    bar.set_title("Revenue & Net Income ($k)"); bar.set_width(420); bar.set_height(280);
    bar.set_legend_position(LegendPosition::Bottom);
    let rs = bar.add_series();
    rs.set_values("Data!$B$2:$D$2"); rs.set_categories("Data!$B$1:$D$1");
    rs.set_name("Revenue"); rs.set_color(green); rs.set_data_labels(true);
    let ns = bar.add_series();
    ns.set_values("Data!$B$6:$D$6"); ns.set_categories("Data!$B$1:$D$1");
    ns.set_name("Net Income"); ns.set_color(blue); ns.set_data_labels(true);
    ws2.insert_chart(chart_row, 6, &bar)?;

    ws2.set_landscape(); ws2.set_fit_to_page(1, 1);
    ws2.set_header("&CFinancial Ratios Dashboard"); ws2.set_footer("&CConfidential  |  &D");
