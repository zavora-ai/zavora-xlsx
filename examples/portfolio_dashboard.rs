...
fn main() -> Result<()> {
    ...
    let projects: Vec<Proj> = vec![
        ...
        Proj {
            name: "Riggs-A-lot Decommissioning",
            budget: 377.0,
            spent: 188.0,
            pct: 50,
            months: 3,
        ...
        Proj {
            name: "12th Annual Customer Convention",
            budget: 490.0,
            spent: 343.0,
            pct: 70,
            months: 11,
    ...
    // ═══════════════════════════════════════════════════════════
    {
        ...
        for (c, hdr) in h.iter().enumerate() {
            ws.write_with_format(0, c as u16, *hdr, &hf)?;
        }
    ...
    let cw = [
        2.0, 4.0, 22.0, 12.0, 10.0, 10.0, 10.0, 10.0, 4.0, 14.0, 14.0, 14.0, 14.0, 14.0, 2.0,
    ];
    ...
    let _kpi_num = Format::new()
        .bold()
        .font_size(22.0)
        .font_color(dark)
        .align(Align::Center)
        .align(Align::VerticalCenter);
    let _kpi_money = Format::new()
        .bold()
        .font_size(18.0)
        .font_color(dark)
        .align(Align::Center)
    ...
    let _stat_label = Format::new()
        .font_size(8.0)
        .font_color("#7F8C8D")
        .align(Align::Center);
    let stat_icon_green = Format::new()
        .font_size(10.0)
        .font_color(green)
        .align(Align::Center);
    let stat_icon_red = Format::new()
        .font_size(10.0)
        .font_color(red)
        .align(Align::Center);
    let stat_icon_amber = Format::new()
        .font_size(10.0)
        .font_color(amber)
        .align(Align::Center);

    let _section = Format::new()
        .bold()
        .font_size(12.0)
        .font_color(white)
        .background_color(navy)
    ...
    for (i, p) in projects.iter().enumerate() {
        ...
        ws2.set_row_height(r, 16.0)?;
        r += 1;

        // Spacer between cards
        ws2.set_row_height(r, 4.0)?;
        r += 1;
    }
    ...
    let chart_colors = [
        blue, "#ED7D31", amber, green, "#5B9BD5", red, "#7B1FA2", "#00B0F0",
    ];
    for (i, c) in chart_colors.iter().enumerate() {
        if i < projects.len() {
            ds.set_point_color(i, *c);
        }
    }
    ...
    ws2.write_with_format(
        ...
        1,
        "LEGEND:",
        &Format::new().bold().font_size(8.0).font_color(dark),
    )?;
    ws2.write_with_format(
        legend_row,
        2,
        "● On Track",
        &Format::new().font_size(8.0).font_color(green),
    )?;
    ws2.write_with_format(
        legend_row,
        3,
        "▲ At Risk",
        &Format::new().font_size(8.0).font_color(amber),
    )?;
    ws2.write_with_format(
        legend_row,
        4,
        "✖ Critical",
        &Format::new().font_size(8.0).font_color(red),
    )?;
    ws2.write_with_format(
        legend_row + 1,
        1,
        "👥 People  📋 Activities  ⚠ Issues  🔴 Risks",
        &legend_fmt,
    )?;
