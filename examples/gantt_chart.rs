...
fn main() -> Result<()> {
    ...
    let border = "#D6DCE4";
    let colors = [
        "#4472C4", "#ED7D31", "#70AD47", "#FFC000", "#5B9BD5", "#C00000", "#7B1FA2", "#00B0F0",
    ];
    ...
    for w in 0..num_weeks {
        ws.set_column_width((7 + w) as u16, 4.0)?;
    }
    ...
    let phase_fmt = |ci: usize| -> Format {
        Format::new()
            ...
            .border_color(colors[ci % colors.len()])
    };
    ...
    ws.write_with_format(
        r,
        1,
        "ERP Implementation  |  Apr – Jul 2025  |  5 Phases, 21 Tasks",
        &subtitle_fmt,
    )?;
    r += 1;
    for c in 1..=(6 + num_weeks as u16) {
        ws.write_with_format(r, c, "", &divider_fmt)?;
    }
    ...
    for w in 0..num_weeks {
        ...
        for (mi, &ms) in month_starts.iter().enumerate() {
            let me = if mi + 1 < month_starts.len() {
                month_starts[mi + 1]
            } else {
                proj_end + 1
            };
    ...
    for phase in &phases {
        ...
        for w in 0..num_weeks {
            ws.write_with_format(r, (7 + w) as u16, "", &pf)?;
        }
        ...
        for task in &phase.tasks {
            ...
            ws.write_with_format(
                r,
                3,
                ExcelDateTime::from_ymd(task.start.0, task.start.1, task.start.2).unwrap(),
                &date_fmt,
            )?;
            ws.write_with_format(
                r,
                4,
                ExcelDateTime::from_ymd(task.end.0, task.end.1, task.end.2).unwrap(),
                &date_fmt,
            )?;
            ...
            let bar_full = Format::new()
                .background_color(bar_color)
                .border(BorderStyle::Thin)
                .border_color(bar_color);
            let bar_progress = Format::new()
                .background_color(navy)
                .border(BorderStyle::Thin)
                .border_color(navy);
    ...
    for (c, h) in sh.iter().enumerate() {
        ws2.write_with_format(r2, (c + 1) as u16, *h, &hdr_fmt)?;
    }
    ws2.write_with_format(r2, 1, sh[0], &hdr_left)?;
    ws2.set_row_height(r2, 22.0)?;
    r2 += 1;
    ...
    for phase in &phases {
        let pf = Format::new()
            .bold()
            .font_size(10.0)
            .font_color(colors[phase.color % colors.len()])
            .align(Align::Left)
            .border(BorderStyle::Thin)
            .border_color(border);
        let nf = Format::new()
        ...
        let avg_pct: f64 = phase.tasks.iter().map(|t| t.pct as f64).sum::<f64>()
            / phase.tasks.len() as f64
            / 100.0;
        let earliest = phase
            .tasks
            .iter()
            .map(|t| serial(t.start.0, t.start.1, t.start.2))
            .min()
            .unwrap();
        let latest = phase
            .tasks
            .iter()
            .map(|t| serial(t.end.0, t.end.1, t.end.2))
            .max()
            .unwrap();
        let status = if avg_pct >= 1.0 {
            "✅ Complete"
        } else if avg_pct > 0.0 {
            "🔄 In Progress"
        } else {
    ...
    ws2.add_conditional_format(
        ...
        r2 - 1,
        3,
        ConditionalFormatDataBar::new("#70AD47"),
    )?;
    ...
    s.set_values(&format!(
        "Summary!$D${}:$D${}",
        summary_start + 1,
        summary_start + phases.len() as u32
    ));
    ...
    let path = std::path::PathBuf::from("output/gantt_chart.xlsx");
    wb.save(&path)?;
