...
fn main() -> Result<()> {
    ...
    let dark = "#333333";
    let gray = "#7F8C8D";
    let green_bar = "#27AE60";
    let blue_bar = "#2980B9";
    let amber_bar = "#F39C12";
    let red_bar = "#E74C3C";
    let purple_bar = "#8E44AD";
    let teal_bar = "#1ABC9C";
    let border = "#D5D8DC";
    let light = "#F8F9FA";
    ...
    let cw = [
        2.0, 22.0, 10.0, 10.0, 3.0, 22.0, 10.0, 10.0, 22.0, 10.0, 10.0, 22.0, 10.0, 10.0,
    ];
    ...
    let section_total = Format::new()
        .bold()
        .font_size(12.0)
        .font_color(dark)
        .align(Align::Right)
        .num_format("$#,##0");
    let section_freq = Format::new()
        .font_size(10.0)
        .font_color(orange)
        .italic()
        .align(Align::Left);
    ...
    // ── Title ──
    for c in 0..=13u16 {
        ws.write_with_format(r, c, "", &title_bar)?;
    }
    ws.write_with_format(r, 1, "Simple Home Budget", &title)?;
    ws.set_row_height(r, 36.0)?;
    r += 2;
    ...
    for (name, val, f) in &savings {
        ...
        if *val > 0.0 {
            ws.write_with_format(r, 2, *val, &amount)?;
        } else {
            ws.write_with_format(r, 2, "", &empty_row)?;
        }
    ...
    let categories: Vec<Cat> = vec![
        Cat {
            ...
            color: green_bar,
            items: vec![
                ("Public Transport", 16.0, "Weekly"),
                ("Fuel", 50.0, "Monthly"),
                ("Bought Lunches", 10.0, "Fortnightly"),
                ("Groceries", 150.0, "Weekly"),
                ("Takeaways", 30.0, "Monthly"),
                ("Dining Out", 70.0, "Monthly"),
        ...
        Cat {
            ...
            color: blue_bar,
            items: vec![
                ("Mortgage", 1380.0, "Fortnightly"),
                ("Power", 50.0, "Monthly"),
                ("Gas / Wood", 50.0, "Monthly"),
                ("Water", 0.0, "Monthly"),
                ("Internet", 90.0, "Monthly"),
                ("Cell phone", 50.0, "Monthly"),
                ("Property Tax", 3800.0, "Yearly"),
                ("Garden Maintenance", 20.0, "Monthly"),
                ("House Maintenance", 50.0, "Monthly"),
                ("Cleaning", 10.0, "Monthly"),
                ("Laundry", 5.0, "Monthly"),
            ],
        },
        Cat {
            ...
            items: vec![
                ("Child Support", 0.0, ""),
                ("Elder Care", 0.0, ""),
                ("Paid TV", 12.0, "Monthly"),
                ("Bank Fees", 20.0, "Yearly"),
    ...
    for cat in &categories {
        ...
        if cat.col == 5 {
            ws.write_with_format(
                ...
                c,
                "Your Expenditure",
                &Format::new().bold().font_size(11.0).font_color("#5D6D7E"),
            )?;
        }
        cr += 1;
        ...
        ws.write_with_format(
            ...
            c,
            cat.name,
            &Format::new().bold().font_size(12.0).font_color(dark),
        )?;
        ...
        // Color bar
        for ci in 0..3u16 {
            ws.write_with_format(cr, c + ci, "", &color_bar(cat.color))?;
        }
        ...
        for (name, val, f) in &cat.items {
            ...
            if *val > 0.0 {
                ws.write_with_format(cr, c + 1, *val, &amount)?;
            } else {
    ...
    let categories2: Vec<Cat> = vec![
        Cat {
            ...
            color: amber_bar,
            items: vec![
                ("Home Insurance", 1200.0, "Yearly"),
                ("Body corp. fees", 0.0, ""),
                ("Vehicle insurance", 9.0, "Monthly"),
                ("Vehicle maintenance", 400.0, "Yearly"),
                ("Registration / licensing", 200.0, "Yearly"),
                ("Warrant of Fitness", 60.0, "Yearly"),
                ("Breakdown cover", 0.0, ""),
                ("Contents insurance", 0.0, ""),
                ("Doctors", 50.0, "Monthly"),
                ("Health products", 20.0, "Monthly"),
                ("Gifts / Christmas", 30.0, "Monthly"),
                ("Vacation", 4000.0, "Yearly"),
            ],
        },
        Cat {
            name: "Personal",
            ...
            color: purple_bar,
            items: vec![
                ("Personal cash", 25.0, "Monthly"),
                ("Clothing / shoes", 200.0, "Yearly"),
                ("Hair & beauty", 20.0, "Monthly"),
                ("Charity", 20.0, "Monthly"),
        ...
        Cat {
            ...
            color: teal_bar,
            items: vec![
                ("After school care", 0.0, ""),
                ("Pocket money", 30.0, "Monthly"),
                ("School trips", 0.0, ""),
                ("School fees", 500.0, "Yearly"),
                ("Uniforms", 250.0, "Yearly"),
                ("Activities / learning", 500.0, "Yearly"),
    ...
    ws.write_formula(
        balance_row,
        2,
        &format!("{income_cell}-{savings_total}-{expense_sum}"),
    )?;
    ...
    ws.write_with_format(
        ...
        &Format::new().font_size(14.0).font_color(dark).italic(),
    )?;
    ...
    let all_colors = [
        "#2ECC71", green_bar, blue_bar, red_bar, amber_bar, purple_bar, teal_bar,
    ];
...
fn col_letter(col: u16) -> String {
    zavora_xlsx::utility::col_to_letter(col)
}
