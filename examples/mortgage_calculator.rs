...
fn main() -> Result<()> {
    ...
    let tbl_hdr = Format::new()
        ...
        .align(Align::Center)
        .border(BorderStyle::Thin);
    let cell_n = Format::new()
        .font_size(9.0)
        .font_color(navy)
        .align(Align::Center)
        .border(BorderStyle::Thin)
        .border_color(border);
    let cell_m = Format::new()
        .font_size(9.0)
        .font_color(navy)
        .align(Align::Right)
        .num_format("$#,##0.00")
    ...
    ws.write_with_format(
        r,
        1,
        "Enter your loan details below — all results update automatically",
        &sub,
    )?;
    r += 1;
    for c in 1..=8u16 {
    ...
    ws.write_with_format(
        r,
        2,
        ExcelDateTime::from_ymd(2025, 5, 1).unwrap(),
        &Format::new()
            .font_size(12.0)
            .font_color(blue)
            .bold()
    ...
    let cat_range = format!(
        "'Mortgage Calculator'!$E${}:$E${}",
        sched_start + 1,
        sched_start + 60
    );

    let cs = chart.add_series();
    cs.set_values(&format!(
        "'Mortgage Calculator'!$G${}:$G${}",
        sched_start + 1,
        sched_start + 60
    ));
    ...
    is.set_values(&format!(
        "'Mortgage Calculator'!$H${}:$H${}",
        sched_start + 1,
        sched_start + 60
    ));
