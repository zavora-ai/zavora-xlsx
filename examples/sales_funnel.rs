...
fn main() -> Result<()> {
    ...
    let header_fmt = Format::new()
        ...
        .background_color(header_bg).align(Align::Left).align(Align::VerticalCenter)
        .border(BorderStyle::Thin).border_color(header_bg);
    let header_center = Format::new()
        .bold().font_size(10.0).font_color("#FFFFFF")
        .background_color(header_bg).align(Align::Center).align(Align::VerticalCenter)
        .border(BorderStyle::Thin).border_color(header_bg);

    let stage_fmt = |alt: bool| -> Format {
        Format::new().font_size(10.0).font_color("#2B579A")
    ...
    let value_fmt = |alt: bool| -> Format {
        Format::new().font_size(10.0).font_color("#333333")
            .align(Align::Right).align(Align::VerticalCenter)
            .num_format("$ #,##0")
            .background_color(if alt { alt_bg } else { "#FFFFFF" })
            .border(BorderStyle::Thin).border_color(border_clr)
    };
    let bar_cell_fmt = |alt: bool| -> Format {
        Format::new().font_size(11.0).font_color(bar_color)
    ...
    let conv_fmt = |alt: bool, is_high: bool| -> Format {
        Format::new().font_size(10.0)
            .font_color(if is_high { "#0D7C3D" } else { "#C00000" })
            .bold().align(Align::Center).align(Align::VerticalCenter)
            .num_format("0%")
            .background_color(if alt { alt_bg } else { "#FFFFFF" })
            .border(BorderStyle::Thin).border_color(border_clr)
    };
    ...
    // ── Summary Section ──
    let sr = last_row + 2;
    let summary_label = Format::new().bold().font_size(10.0).font_color("#333333").align(Align::Right);
    let summary_value = Format::new().bold().font_size(12.0).font_color("#0D7C3D").align(Align::Left);
    let summary_pct = Format::new().bold().font_size(12.0).font_color("#0D7C3D").align(Align::Left).num_format("0.0%");
    ...
    let path = std::path::PathBuf::from(std::env::var("HOME").unwrap_or("/tmp".into()))
        .join("Downloads/sales_funnel_incell.xlsx");
    wb.save(&path)?;
