...
use zavora_xlsx::formula_engine::evaluator::{SimpleContext, Value, evaluate};
...
fn main() {
    ...
    let vlookup_formulas = [
        ...
        (
            ...
            "Exact match: lookup 2 → price 0.30",
        ),
        (
            "VLOOKUP(2.5,A1:C4,2,TRUE)",
            "Approx match: 2.5 → Banana (largest ≤ 2.5)",
        ),
    ...
    let index_match_formulas = [
        ...
        (
            ...
            "Exact MATCH for Cherry → position 3",
        ),
        (
            "INDEX(C1:C4,MATCH(\"Cherry\",B1:B4,0))",
            "INDEX+MATCH → price of Cherry",
        ),
    ];
    ...
    let combined = [
        ("SUM(A1:A5)*2", "Sum * 2"),
        (
            "IF(AVERAGE(C1:C4)>0.5,\"expensive\",\"cheap\")",
            "Conditional on average price",
        ),
