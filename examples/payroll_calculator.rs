...
fn main() -> Result<()> {
    ...
    r += 1;
    ws.write_with_format(r, 1, "💰 Payroll Calculator", &title)?;
    ws.set_row_height(r, 32.0)?;
    r += 1;
    ws.write_with_format(
        r,
        1,
        "Monthly Payroll — April 2025  |  All deductions calculated by formula",
    ...
    ws.write_with_format(r, 1, headers[0], &hdr_l)?;
    ws.write_with_format(r, 2, headers[1], &hdr_l)?;
    for (c, h) in headers[2..].iter().enumerate() {
        ws.write_with_format(r, (c + 3) as u16, *h, &hdr)?;
    }
    ws.set_row_height(r, 22.0)?;
    r += 1;

    let staff: Vec<(&str, &str, f64)> = vec![
        ("Sarah Kimani", "Finance", 185000.0),
        ("James Mwangi", "Engineering", 165000.0),
        ("Amina Wanjiku", "Marketing", 140000.0),
        ("David Ochieng", "Engineering", 120000.0),
        ("Grace Njeri", "HR", 110000.0),
        ("Peter Lumumba", "Engineering", 95000.0),
        ("Faith Akinyi", "Finance", 85000.0),
        ("Brian Kipchoge", "Sales", 125000.0),
        ("Lucy Wambui", "Marketing", 88000.0),
        ("Kevin Otieno", "Engineering", 72000.0),
        ("Mary Auma", "HR", 58000.0),
        ("John Kamau", "Sales", 68000.0),
    ];
    ...
    for (i, (name, dept, gross)) in staff.iter().enumerate() {
        ...
        // Tax = 30% of gross
        ws.write_formula(r, 4, &format!("D{}*0.30", r + 1))?;
        ws.set_cell_format(r, 4, &mf)?;
        // NSSF = fixed 2160
        ...
        ws.write_formula(
            r,
            6,
            &format!("IF(D{r1}>12000,1700,IF(D{r1}>5000,1200,600))", r1 = r + 1),
        )?;
        ws.set_cell_format(r, 6, &mf)?;
        // Pension = 5%
        ws.write_formula(r, 7, &format!("D{}*0.05", r + 1))?;
        ws.set_cell_format(r, 7, &mf)?;
        // Total Deductions
        ws.write_formula(
            r,
            8,
            &format!("E{}+F{}+G{}+H{}", r + 1, r + 1, r + 1, r + 1),
        )?;
        ws.set_cell_format(r, 8, &mf)?;
        // Net Pay
        ws.write_formula(r, 9, &format!("D{}-I{}", r + 1, r + 1))?;
        ws.set_cell_format(r, 9, &net_f)?;
        r += 1;
    }
    let data_end = r - 1;
    ...
    ws.set_header("&CPayroll — April 2025  |  Confidential");
