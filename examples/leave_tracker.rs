...
fn main() -> Result<()> {
    ...
    let cw = [
        2.0, 18.0, 14.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0,
    ];
    ...
    r += 1;
    ws.write_with_format(r, 1, "🏖️ Leave Tracker", &title)?;
    ws.set_row_height(r, 32.0)?;
    r += 1;
    ws.write_with_format(
        r,
        1,
        "Annual Leave Management — 2025",
    ...
    for (i, (name, dept, annual, sick, personal, used, pending)) in staff.iter().enumerate() {
        ...
        ws.write_formula(
            r,
            8,
            &format!("(D{}+E{}+F{})-G{}-H{}", r + 1, r + 1, r + 1, r + 1, r + 1),
        )?;
        ...
        ws.write_formula(
            r,
            9,
            &format!("IFERROR(G{}/(D{}+E{}+F{}),0)", r + 1, r + 1, r + 1, r + 1),
        )?;
        ws.set_cell_format(
            r,
            9,
    ...
    ws.write_formula(r, 6, &format!("SUM(G{}:G{})", data_start + 1, data_end + 1))?;
    ...
    ws.write_formula(
        r,
        6,
        &format!("AVERAGE(J{}:J{})", data_start + 1, data_end + 1),
    )?;
    ws.set_cell_format(
        r,
        6,
    ...
    ws.set_header("&CLeave Tracker — 2025");
