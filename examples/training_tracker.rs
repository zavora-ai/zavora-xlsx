...
fn main() -> Result<()> {
    ...
    let headers = ["Program","Department","Provider","Month","Day","Duration",
                   "Cost","Capacity","Enrolled","Attended","Fill Rate","Rating"];
    let hdr_fmt = Format::new().bold().font_size(10.0).font_color("#FFFFFF")
        .background_color("#C68E17").align(Align::Center).border(BorderStyle::Thin);
    for (c, h) in headers.iter().enumerate() {
        ws.write_with_format(0, c as u16, *h, &hdr_fmt)?;
    }

    let programs: Vec<[&str; 10]> = vec![
        ["Advanced People Management","HR","Libero Associates","October","15","1","5000","20","6","3.5"],
        ["Advanced IT Techniques","IT","Nulla Eu Inc","November","9","1","5000","30","6","0"],
        ["Advanced IT Skills","IT","Vivamus Industries","December","1","1.5","3000","15","6","0"],
        ["Advanced Business Techniques","Sales","Eu Dolor Egestas","December","14","1","3000","20","1","0"],
        ["Leadership Essentials","Finance","Apex Training","January","20","2","8000","25","22","4.5"],
        ["Digital Marketing Masterclass","Marketing","DigiLearn Corp","January","27","1","4500","30","28","4"],
        ["Supply Chain Optimization","Operations","LogiTrain Partners","February","10","2","7500","20","18","4.2"],
        ["Excel for Finance Pros","Finance","DataSkills Academy","February","24","1","3500","25","25","4.8"],
        ["Conflict Resolution Workshop","HR","PeopleSoft Training","March","5","0.5","2500","15","14","3.8"],
        ["Cybersecurity Fundamentals","IT","SecureNet Institute","March","17","2","9000","20","20","4.6"],
        ["Sales Negotiation Tactics","Sales","CloseRate Academy","March","24","1","5500","25","24","4.3"],
        ["Project Management Bootcamp","Operations","PMI Trainers","April","7","3","12000","20","20","4.7"],
    ];

    let txt = Format::new().font_size(10.0).border(BorderStyle::Thin).border_color("#E0D5B0");
    let num = Format::new().font_size(10.0).align(Align::Center).border(BorderStyle::Thin).border_color("#E0D5B0");
    let money = Format::new().font_size(10.0).align(Align::Center).num_format("$ #,##0")
        .border(BorderStyle::Thin).border_color("#E0D5B0");
    let pct = Format::new().font_size(10.0).align(Align::Center).num_format("0%")
        .border(BorderStyle::Thin).border_color("#E0D5B0");
    ...
    // Column widths
    let widths = [30.0, 14.0, 22.0, 12.0, 6.0, 8.0, 12.0, 10.0, 10.0, 10.0, 10.0, 8.0];
    for (c, w) in widths.iter().enumerate() { ws.set_column_width(c as u16, *w)?; }

    // Add table for filtering
    ws.add_table(0, 0, last as u32, 11, &Table::new()
        .set_style(TableStyle::Medium(4))
        .set_columns(&headers.iter().map(|h| TableColumn::new(h)).collect::<Vec<_>>()))?;
    ...
    for (c, w) in dw.iter().enumerate() { ws2.set_column_width(c as u16, *w)?; }
    ...
    let title_fmt = Format::new().bold().font_size(22.0).font_color("#FFFFFF")
        .background_color(gold).align(Align::Left).align(Align::VerticalCenter);
    let title_bar = Format::new().background_color(gold);
    let section_green = Format::new().bold().font_size(11.0).font_color("#FFFFFF")
        .background_color(green).align(Align::Center).border(BorderStyle::Thin);
    let section_orange = Format::new().bold().font_size(11.0).font_color("#FFFFFF")
        .background_color(gold).align(Align::Center).border(BorderStyle::Thin);
    let kpi_label = Format::new().bold().font_size(9.0).font_color("#666666").align(Align::Center);
    let kpi_val = Format::new().bold().font_size(16.0).font_color(navy).align(Align::Center);
    let kpi_money = Format::new().bold().font_size(16.0).font_color(navy).align(Align::Center).num_format("$ #,##0");
    let kpi_pct = Format::new().bold().font_size(16.0).font_color(green).align(Align::Center).num_format("0%");
    let dept_hdr = Format::new().bold().font_size(10.0).font_color("#FFFFFF")
        .background_color(dark_gold).align(Align::Center).border(BorderStyle::Thin);
    let dept_val = Format::new().font_size(10.0).font_color(navy).align(Align::Center)
        .border(BorderStyle::Thin).border_color("#E0D5B0");
    let dept_money = Format::new().font_size(10.0).font_color(navy).align(Align::Center)
        .num_format("$ #,##0").border(BorderStyle::Thin).border_color("#E0D5B0");
    let dept_pct = Format::new().font_size(10.0).font_color(navy).align(Align::Center)
        .num_format("0%").border(BorderStyle::Thin).border_color("#E0D5B0");
    let dept_name = Format::new().bold().font_size(10.0).font_color(navy).align(Align::Left)
        .border(BorderStyle::Thin).border_color("#E0D5B0");

    let mut r = 0u32;

    // Title
    for c in 0..=8u16 { ws2.write_with_format(r, c, "", &title_bar)?; }
    ws2.write_with_format(r, 1, "Training Dashboard", &title_fmt)?;
    ws2.set_row_height(r, 44.0)?; r += 2;

    // ── KPI Section (all formulas) ──
    ws2.merge_range(r, 1, r, 4, "People", &section_green)?;
    ws2.merge_range(r, 5, r, 8, "Courses", &section_orange)?;
    ws2.set_row_height(r, 24.0)?; r += 1;

    // KPI labels
    let labels = ["Total Enrolled", "Avg per Course", "Max Enrolled", "Min Enrolled",
                  "Total Programs", "Total Cost", "Total Capacity", "Avg Fill Rate"];
    ...
    ws2.set_row_height(r, 30.0)?; r += 2;

    // ── Department Breakdown (all COUNTIFS/SUMIFS/AVERAGEIFS) ──
    ws2.merge_range(r, 1, r, 8, "Department Breakdown", &section_orange)?;
    ws2.set_row_height(r, 24.0)?; r += 1;

    let dept_headers = ["Department", "Programs", "Total Cost", "Avg Cost", "Total Enrolled", "Avg Fill Rate", "Avg Rating", "Over-subscribed"];
    ...
    for dept in &departments {
        ...
        ws2.write_formula(r, 7, &format!("AVERAGEIFS(Data!L2:L{last},{dept_range},\"{dept}\",Data!L2:L{last},\">0\")"))?;
        ws2.set_cell_format(r, 7, &dept_val)?;
        // Over-subscribed = COUNTIFS(dept, fill > 100%)
        ws2.write_formula(r, 8, &format!("COUNTIFS({dept_range},\"{dept}\",{kr},\">1\")"))?;
    ...
    let dept_end = r - 1;
    let total_fmt = Format::new().bold().font_size(10.0).font_color(navy).align(Align::Center)
        .background_color("#FFF8E7").border(BorderStyle::Thin).border_color("#E0D5B0");
    let total_money = Format::new().bold().font_size(10.0).font_color(navy).align(Align::Center)
        .num_format("$ #,##0").background_color("#FFF8E7").border(BorderStyle::Thin).border_color("#E0D5B0");
    let total_pct = Format::new().bold().font_size(10.0).font_color(green).align(Align::Center)
        .num_format("0%").background_color("#FFF8E7").border(BorderStyle::Thin).border_color("#E0D5B0");
    ...
    ws2.write_formula(r, 4, &format!("AVERAGE(E{}:E{})", dept_start + 1, dept_end + 1))?;
    ...
    ws2.write_formula(r, 6, &format!("AVERAGE(G{}:G{})", dept_start + 1, dept_end + 1))?;
    ws2.set_cell_format(r, 6, &total_pct)?;
    ws2.write_formula(r, 7, &format!("AVERAGE(H{}:H{})", dept_start + 1, dept_end + 1))?;
    ...
    s1.set_values(&format!("Dashboard!$D${}:$D${}", dept_start + 1, dept_end + 1));
    s1.set_categories(&format!("Dashboard!$B${}:$B${}", dept_start + 1, dept_end + 1));
    ...
    s2.set_values(&format!("Dashboard!$G${}:$G${}", dept_start + 1, dept_end + 1));
    s2.set_categories(&format!("Dashboard!$B${}:$B${}", dept_start + 1, dept_end + 1));
    ...
    ws2.add_conditional_format(dept_start, 6, dept_end, 6,
