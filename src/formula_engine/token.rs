...
pub enum Token {
    ...
    SheetRef { sheet: String, inner: Box<Token> },
...
fn parse_identifier(
    ...
    if !ident.is_empty() {
        ...
        if upper.starts_with('R')
            && (upper.len() > 1 || (*i < len && (chars[*i] == '[' || chars[*i].is_ascii_digit())))
        {
...
fn try_parse_cell_ref(s: &str) -> Option<Token> {
    ...
    let abs_col = if pos < len && bytes[pos] == b'$' {
        pos += 1;
        true
    } else {
        false
    ...
    let abs_row = if pos < len && bytes[pos] == b'$' {
        pos += 1;
        true
    } else {
        false
...
mod tests {
    ...
    fn test_string_with_escaped_quotes() {
        ...
        assert_eq!(
            tokens,
            vec![Token::StringLiteral(r#"He said "hi"!"#.into())]
        );
    }
    ...
    fn test_percent_operator() {
        ...
        assert_eq!(
            tokens,
            vec![Token::Number(50.0), Token::Operator(Op::Percent)]
        );
    }
