...
use super::Worksheet;
use super::types::{Comment, Hyperlink, PrintSettings, SheetProtection, hash_password};
...
impl Worksheet {
    ...
    pub fn add_conditional_format(
        ...
        r2: RowNum,
        c2: ColNum,
        cf: impl ConditionalFormat + 'static,
    ) -> crate::Result<&mut Self> {
        self.conditional_formats.push(StoredCf {
            range: (r1, c1, r2, c2),
            rule: Box::new(cf),
            dxf_id: None,
        });
    ...
    pub fn add_pivot_table(
        ...
        row: RowNum,
        col: ColNum,
        pivot: &crate::features::pivot::PivotTable,
    ) -> crate::Result<&mut Self> {
        let mut pt = pivot.clone();
        pt.row = row;
        pt.col = col;
    ...
    pub fn insert_treemap(
        &mut self,
        row: RowNum,
        col: ColNum,
        chart: &crate::features::treemap::TreemapChart,
    ) -> crate::Result<&mut Self> {
        let mut tc = chart.clone();
        tc.row = row;
        tc.col = col;
        self.treemap_charts.push(tc);
        self.dirty = true;
        Ok(self)
    }

    /// Insert a waterfall chart (Excel 2016+ ChartEx format).
    pub fn insert_waterfall(
        &mut self,
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::WaterfallChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::Waterfall(c));
        self.dirty = true;
        Ok(self)
    }
    ...
    pub fn insert_funnel(
        ...
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::FunnelChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::Funnel(c));
        self.dirty = true;
        Ok(self)
    }

    /// Insert a sunburst chart (Excel 2016+ ChartEx format).
    pub fn insert_sunburst(
        &mut self,
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::SunburstChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::Sunburst(c));
        self.dirty = true;
        Ok(self)
    }
    ...
    pub fn insert_histogram(
        ...
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::HistogramChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::Histogram(c));
        self.dirty = true;
        Ok(self)
    }

    /// Insert a box & whisker chart (Excel 2016+ ChartEx format).
    pub fn insert_box_whisker(
        &mut self,
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::BoxWhiskerChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::BoxWhisker(c));
        self.dirty = true;
        Ok(self)
    }
    ...
    pub fn insert_map(
        ...
        row: RowNum,
        col: ColNum,
        chart: &crate::features::chartex::MapChart,
    ) -> crate::Result<&mut Self> {
        let mut c = chart.clone();
        c.row = row;
        c.col = col;
        self.chartex_charts
            .push(crate::features::chartex::ChartExChart::Map(c));
        self.dirty = true;
        Ok(self)
    }

    /// Add a slicer visual filter control at the given position.
    pub fn add_slicer(
        &mut self,
        row: RowNum,
        col: ColNum,
    ...
    /// Add a timeline visual filter control at the given position.
    pub fn add_timeline(
        &mut self,
        row: RowNum,
        col: ColNum,
    ...
    /// Protect the sheet with a custom `SheetProtection` configuration.
    ///
    /// Use the builder methods on `SheetProtection` to configure individual
    /// protection flags (sort, autoFilter, formatCells, etc.) before passing
    /// the struct to this method.
    pub fn protect_with_options(&mut self, prot: SheetProtection) -> &mut Self {
    ...
    pub fn unprotect_range(&mut self, name: &str, range: &str) -> &mut Self {
        ...
        ps.protected_ranges
            .push((name.to_string(), range.to_string(), None));
        self.dirty = true;
        self
    }

    pub fn unprotect_range_with_password(
        &mut self,
        name: &str,
        range: &str,
        password: &str,
    ...
    /// Add a threaded comment (modern comment) to a cell.
    pub fn add_threaded_comment(
        &mut self,
        row: RowNum,
        col: ColNum,
        comment: super::types::ThreadedComment,
    ) -> &mut Self {
        let mut tc = comment;
        tc.row = row;
        tc.col = col;
        self.threaded_comments.push(tc);
        self.dirty = true;
        self
    }

    /// Add a form control at the given cell position.
    pub fn add_form_control(
        &mut self,
        row: RowNum,
        col: ColNum,
        control: super::types::FormControl,
    ) -> &mut Self {
        self.form_controls.push((row, col, control));
        self.dirty = true;
        self
    }

    /// Add a drawing shape at the given cell position.
    pub fn add_shape(
        &mut self,
        row: RowNum,
        col: ColNum,
