...
pub trait ConditionalFormat: Send + Sync {
    ...
    fn write_rule(
        &self,
        w: &mut crate::xml::xml_writer::XmlWriter,
        priority: u32,
        dxf_id: Option<u32>,
    );
    fn dxf_format(&self) -> Option<&Format> {
        None
    }
}
...
impl CfOperator {
    pub fn xml_str(&self) -> &str {
        match self {
            ...
            CfOperator::EqualTo => "equal",
            CfOperator::NotEqualTo => "notEqual",
...
impl ConditionalFormatCell {
    ...
    pub fn set_value2(&mut self, v: f64) -> &mut Self {
        self.value2 = Some(v);
        self
    }
    pub fn set_format(&mut self, f: &Format) -> &mut Self {
        self.format = Some(f.clone());
        self
    }
}
...
impl ConditionalFormat for ConditionalFormatCell {
    ...
    fn write_rule(
        ...
        let mut attrs: Vec<(&str, &str)> = vec![
            ("type", "cellIs"),
            ("priority", &p),
            ("operator", self.operator.xml_str()),
        ];
...
impl ConditionalFormat2ColorScale {
    pub fn new(min: impl crate::format::IntoColor, max: impl crate::format::IntoColor) -> Self {
        Self {
            min_color: min.into_color().to_rgb(),
            max_color: max.into_color().to_rgb(),
        }
    }
}
...
impl ConditionalFormat for ConditionalFormat2ColorScale {
    ...
    fn write_rule(
        ...
        let min_hex = format!(
            "FF{:02X}{:02X}{:02X}",
            self.min_color[0], self.min_color[1], self.min_color[2]
        );
        let max_hex = format!(
            "FF{:02X}{:02X}{:02X}",
            self.max_color[0], self.max_color[1], self.max_color[2]
        );
...
impl ConditionalFormat3ColorScale {
    pub fn new(
        min: impl crate::format::IntoColor,
        mid: impl crate::format::IntoColor,
        max: impl crate::format::IntoColor,
    ) -> Self {
        Self {
            min_color: min.into_color().to_rgb(),
            mid_color: mid.into_color().to_rgb(),
            max_color: max.into_color().to_rgb(),
        }
    }
}
...
impl ConditionalFormat for ConditionalFormat3ColorScale {
    ...
    fn write_rule(
        ...
        let min_h = c(self.min_color);
        let mid_h = c(self.mid_color);
        let max_h = c(self.max_color);
...
impl ConditionalFormatDataBar {
    pub fn new(color: impl crate::format::IntoColor) -> Self {
        Self {
            color: color.into_color().to_rgb(),
            gradient: false,
        }
    }

    pub fn set_gradient(&mut self, v: bool) -> &mut Self {
        self.gradient = v;
        self
    }
}
...
impl ConditionalFormat for ConditionalFormatDataBar {
    ...
    fn write_rule(
        ...
        w.end_tag("dataBar");
        // When gradient is false, emit gradient="0" in extLst to override Excel's default (gradient=true)
        if !self.gradient {
            w.start_tag("extLst", &[]);
            w.start_tag("ext", &[("uri", "{B025F937-C7B1-47D3-B67F-A62EFF666E3E}")]);
            w.empty_tag("x14:dataBar", &[("gradient", "0")]);
            w.end_tag("ext");
            w.end_tag("extLst");
        }
        w.end_tag("cfRule");
...
impl IconSetType {
    pub fn xml_str(&self) -> &str {
        match self {
            ...
            IconSetType::ThreeTrafficLights => "3TrafficLights",
...
impl ConditionalFormatTopBottom {
    pub fn new(kind: TopBottomType, rank: u32) -> Self {
        Self {
            kind,
            rank,
            format: None,
...
impl ConditionalFormat for ConditionalFormatTopBottom {
    ...
    fn write_rule(
        ...
        let bottom = matches!(
            self.kind,
            TopBottomType::Bottom | TopBottomType::BottomPercent
        );
        let percent = matches!(
            self.kind,
            TopBottomType::TopPercent | TopBottomType::BottomPercent
        );
...
impl ConditionalFormatText {
    pub fn new(op: TextOperator, text: &str) -> Self {
        Self {
            operator: op,
            text: text.into(),
            format: None,
...
impl ConditionalFormat for ConditionalFormatText {
    ...
    fn write_rule(
        ...
        let mut attrs: Vec<(&str, &str)> =
            vec![("type", cf_type), ("priority", &p), ("text", &self.text)];
        if let Some(ref ds) = d {
            attrs.push(("dxfId", ds));
        }
...
impl Default for ConditionalFormatDuplicate {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone)]
pub struct ConditionalFormatUnique {
    pub(crate) format: Option<Format>,
}

impl Default for ConditionalFormatUnique {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ConditionalFormatAverage {
    pub fn new(kind: AverageType) -> Self {
...
impl ConditionalFormat for ConditionalFormatAverage {
    ...
    fn write_rule(
        ...
        match self.kind {
            ...
            AverageType::AboveOrEqual => {
                attrs.push(("equalAverage", "1"));
            }
            AverageType::BelowOrEqual => {
                attrs.push(("aboveAverage", "0"));
                attrs.push(("equalAverage", "1"));
            }
...
impl DateOccurring {
    pub fn xml_str(&self) -> &str {
        match self {
            ...
            DateOccurring::Last7Days => "last7Days",
            DateOccurring::ThisWeek => "thisWeek",
            DateOccurring::LastWeek => "lastWeek",
            DateOccurring::NextWeek => "nextWeek",
            DateOccurring::ThisMonth => "thisMonth",
            DateOccurring::LastMonth => "lastMonth",
            DateOccurring::NextMonth => "nextMonth",
        }
    }
}
...
impl ConditionalFormatDate {
    pub fn new(period: DateOccurring) -> Self {
        Self {
            period,
            format: None,
        }
...
impl ConditionalFormat for ConditionalFormatDate {
    ...
    fn write_rule(
        ...
        let mut attrs: Vec<(&str, &str)> =
            vec![("type", "timePeriod"), ("priority", &p), ("timePeriod", tp)];
        if let Some(ref ds) = d {
            attrs.push(("dxfId", ds));
        }
        w.start_tag("cfRule", &attrs);
