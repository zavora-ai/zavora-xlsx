...
/// Date grouping levels for pivot table field grouping.
...
/// Field grouping configuration for pivot tables.
#[derive(Debug, Clone)]
pub enum FieldGrouping {
    /// Date-based grouping with specified levels.
    Date { levels: Vec<DateGroupLevel> },
    /// Numeric range grouping with start, end, and interval.
    Range { start: f64, end: f64, interval: f64 },
}

/// A calculated item in a pivot table.
#[derive(Debug, Clone)]
...
impl Default for PivotStyle {
    fn default() -> Self {
        Self::Named("PivotStyleLight16".into())
    }
}
...
pub struct PivotTable {
    ...
    pub(crate) calculated_items: Vec<CalculatedItem>,
    pub(crate) field_groupings: Vec<(String, FieldGrouping)>,
...
impl PivotTable {
    ...
    pub fn add_row_field(mut self, field: &str) -> Self {
        self.row_fields.push(field.into());
        self
    }
    pub fn add_column_field(mut self, field: &str) -> Self {
        self.column_fields.push(field.into());
        self
    }
    pub fn add_filter_field(mut self, field: &str) -> Self {
        self.filter_fields.push(field.into());
        self
    }

    pub fn add_value_field(mut self, field: &str, agg: PivotAggregation) -> Self {
        let prefix = match agg {
            PivotAggregation::Sum => "Sum of",
            PivotAggregation::Count => "Count of",
            PivotAggregation::Average => "Average of",
            PivotAggregation::Max => "Max of",
            PivotAggregation::Min => "Min of",
            _ => "Value of",
        };
    ...
    pub fn add_calculated_field(mut self, name: &str, formula: &str) -> Self {
        self.calculated_fields.push((name.into(), formula.into()));
        self
    }

    pub fn set_style_name(mut self, name: &str) -> Self {
        self.style = PivotStyle::Named(name.into());
        self
    }
    pub fn show_row_headers(mut self, v: bool) -> Self {
        self.show_row_headers = v;
        self
    }
    pub fn show_column_headers(mut self, v: bool) -> Self {
        self.show_col_headers = v;
        self
    }
    pub fn show_row_stripes(mut self, v: bool) -> Self {
        self.show_row_stripes = v;
        self
    }
    pub fn show_grand_totals(mut self, rows: bool, cols: bool) -> Self {
        self.show_row_grand_total = rows;
        self.show_col_grand_total = cols;
        self
    }
    pub fn set_layout(mut self, layout: PivotLayout) -> Self {
        self.layout = layout;
        self
    }
    pub fn show_subtotals(mut self, field: &str, show: bool) -> Self {
        self.field_subtotals.push((field.into(), show));
        self
    }
    pub fn set_value_format(mut self, field: &str, fmt: &str) -> Self {
        for vf in &mut self.value_fields {
            if vf.source_field == field {
                vf.num_format = Some(fmt.into());
            }
        }
        self
    }

    /// Group a field by date levels (e.g., Years, Months, Days).
    pub fn group_by_date(mut self, field: &str, levels: &[DateGroupLevel]) -> Self {
        self.field_groupings.push((
            field.into(),
            FieldGrouping::Date {
    ...
    /// Group a field by numeric range with start, end, and interval.
    pub fn group_by_range(mut self, field: &str, start: f64, end: f64, interval: f64) -> Self {
        self.field_groupings.push((
            field.into(),
            FieldGrouping::Range {
                start,
    ...
    /// Add a calculated item to the pivot table.
    pub fn add_calculated_item(mut self, name: &str, formula: &str) -> Self {
        self.calculated_items.push(CalculatedItem {
            name: name.into(),
