        ...
        if b.is_ascii_alphabetic() || b == b'$' {
            ...
            if let Some((_ref_str, end, row, col, abs_row, abs_col)) = try_parse_ref(bytes, i) {
                let new_row = if let Some(at) = at_row {
                    if row >= at {
                        (row as i64 + delta_row).max(0) as RowNum
                    } else {
                        row
                    }
                } else {
                ...
                let new_col = if let Some(at) = at_col {
                    if col >= at {
                        (col as i64 + delta_col).max(0) as ColNum
                    } else {
                        col
                    }
                } else {
...
fn try_parse_ref(
    bytes: &[u8],
    start: usize,
) -> Option<(String, usize, RowNum, ColNum, bool, bool)> {
    let mut i = start;
...
mod tests {
    ...
    fn insert_row_shifts_refs() {
        ...
        assert_eq!(
            adjust_formula("SUM(A1:A5)", Some(1), 1, None, 0),
            "SUM(A1:A6)"
        );
    }
    ...
    fn absolute_refs_shifted() {
        assert_eq!(
            adjust_formula("$A$2+$B$1", Some(1), 1, None, 0),
            "$A$3+$B$1"
        );
    }
    ...
    fn quoted_strings_preserved() {
        assert_eq!(
            adjust_formula("\"A1\"+A1", Some(0), 1, None, 0),
            "\"A1\"+A2"
        );
    }
