...
pub(crate) fn write_data_validation(w: &mut XmlWriter, dv: &DataValidation) {
    let sqref = format!(
        "{}{}:{}{}",
        col_to_letter(dv.first_col),
        dv.first_row + 1,
        col_to_letter(dv.last_col),
        dv.last_row + 1
    );
    ...
    let mut attrs: Vec<(&str, &str)> =
        vec![("type", &dv_type), ("sqref", &sqref), ("allowBlank", "1")];
    let op_str;
    if matches!(
        &dv.rule,
    ...
    if dv.input_title.is_some() {
        attrs.push(("showInputMessage", "1"));
    }
    if dv.error_title.is_some() {
    ...
    if !input_title_ref.is_empty() {
        attrs.push(("promptTitle", input_title_ref));
    }
    if !input_msg_ref.is_empty() {
        attrs.push(("prompt", input_msg_ref));
    }
    if !error_title_ref.is_empty() {
        attrs.push(("errorTitle", error_title_ref));
    }
    if !error_msg_ref.is_empty() {
        attrs.push(("error", error_msg_ref));
    }
...
pub(crate) fn compute_dimension(data: &SheetCells<'_>) -> String {
    ...
    for cols in data.cells.values() {
        if let Some(&c) = cols.keys().next() {
            min_c = min_c.min(c);
        }
        if let Some(&c) = cols.keys().next_back() {
            max_c = max_c.max(c);
        }
    }
    if min_c == u16::MAX {
