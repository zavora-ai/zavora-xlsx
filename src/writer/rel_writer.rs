...
pub fn write_workbook_rels(sheet_count: usize) -> Vec<u8> {
    ...
    let next = sheet_count + 1;
    rels.push((format!("rId{next}"), "http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles", "styles.xml".into()));
    rels.push((format!("rId{}", next + 1), "http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings", "sharedStrings.xml".into()));
