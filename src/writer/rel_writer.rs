...
pub fn write_workbook_rels(sheet_count: usize, has_vba: bool, pivot_count: usize) -> Vec<u8> {
    write_workbook_rels_with_chartsheets(sheet_count, 0, has_vba, pivot_count)
}

pub fn write_workbook_rels_with_chartsheets(
    sheet_count: usize,
    ...
    for i in 0..chartsheet_count {
        rels.push((
            format!("rId{}", sheet_count + i + 1),
            "http://schemas.openxmlformats.org/officeDocument/2006/relationships/chartsheet",
            format!("chartsheets/sheet{}.xml", i + 1),
        ));
    }
    let mut next = sheet_count + chartsheet_count + 1;
    rels.push((
        format!("rId{next}"),
